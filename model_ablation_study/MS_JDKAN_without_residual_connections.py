...
class ContextAwareWavKANBlock(nn.Module):
    ...
    def forward(self, x, debug_store=None, block_idx=None):
        ...
        x_context = self.norm1(x_context) # Không cộng x vào nữa, chỉ chuẩn hóa đầu ra Conv
        ...
        # --- BƯỚC 3: w/o Residual Connection (không cộng ngược x_context) ---
        next_x = self.norm2(kan_out) # Norm trực tiếp trên đầu ra KAN
