...
def _resolve_local_csv_path(root_path, data_path):
    base_name = os.path.basename(data_path)
    stem_name = os.path.splitext(base_name)[0]

    candidate_paths = [
        os.path.join(root_path, data_path),
        os.path.join(root_path, base_name),
    ]

    if not stem_name.startswith("Data_"):
        candidate_paths.append(os.path.join(root_path, f"Data_{stem_name}.csv"))
...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        local_fp = _resolve_local_csv_path(self.root_path, self.data_path)
