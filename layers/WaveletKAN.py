...
class AdaptiveWaveletKANLayer(nn.Module):
    def __init__(self, in_features, out_features, seq_len, num_wavelets=7, wavelet_type='mexican_hat', grid_size=3.0, rank=8):
        ...
        # Khởi tạo 3 ma trận thành phần A, B, D cho phép phân rã CP dùng chung rank
        ...
        # --- Nhánh Trend: trải đều trên toàn miền[cite: 1] ---
        b_wide = torch.linspace(grid_min, grid_max, num_wide)  
        step = (grid_max - grid_min) / max(num_wide - 1, 1)           
        a_wide = torch.ones(num_wide) * step * wide_scale            

        # --- Nhánh Detail: so le (lấp đúng khe giữa các wavelet wide)[cite: 1] ---
        narrow_min = grid_min + step / 2  
        narrow_max = grid_max - step / 2  
        b_narrow = torch.linspace(narrow_min, narrow_max, num_narrow)  
        a_narrow = torch.ones(num_narrow) * step * narrow_scale                

        # --- Tổng hợp Grid[cite: 1] ---
        base_b = torch.cat([b_wide, b_narrow], dim=0)
        grid_b = base_b.unsqueeze(0).repeat(in_features, 1) 
        
        base_a = torch.cat([a_wide, a_narrow], dim=0)
    ...
    def forward(self, x):
        ...
        # phi: [B*C, Seq, in_features, num_wavelets][cite: 1]
        phi = self._compute_wavelet_response(z)
        
        # --- TỐI ƯU HÓA CP-FACTORIZATION (KHATRI-RAO) ---
        
        # Gộp Bước 1 (thu gọn theo trục wavelet) và Bước 2 (thu gọn theo trục chiều ẩn)[cite: 1, 2]
        # Bằng cách sử dụng Khatri-Rao product cho B và D
        KR = (self.B.unsqueeze(1) * self.D.unsqueeze(0)).reshape(-1, self.rank)
        
        # Flatten 2 chiều cuối của phi (in_features và num_wavelets) thành 1 chiều
        # Đầu ra phi_flat: [B*C, Seq, in_features * num_wavelets]
        phi_flat = phi.reshape(*phi.shape[:-2], -1) 
        
        # Thực hiện 1 GEMM duy nhất thay vì 2 bước tuần tự
        # Đầu ra v: [B*C, Seq, rank]
        v = torch.matmul(phi_flat, KR) 
        
        # Bước 3: Thu gọn theo trục rank với ma trận A[cite: 1]
        # Đầu ra out: [B*C, Seq, out_features]
