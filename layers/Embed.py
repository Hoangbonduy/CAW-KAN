...
class TemporalEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        month_x = self.month_embed(x[:, :, 0]) if x.size(2) > 0 else 0.
        day_x = self.day_embed(x[:, :, 1]) if x.size(2) > 1 else 0.
        weekday_x = self.weekday_embed(x[:, :, 2]) if x.size(2) > 2 else 0.
        hour_x = self.hour_embed(x[:, :, 3]) if x.size(2) > 3 else 0.
